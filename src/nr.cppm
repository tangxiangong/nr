...
export template <typename T> class NRVector {
  private:
    // size of array
    size_t m_len = 0;
...
template <class T> inline T &NRVector<T>::operator[](const size_t idx) {
    if (idx >= m_len) {
...
template <class T> inline size_t NRVector<T>::size() const { return m_len; }

template <class T> void NRVector<T>::resize(size_t new_len) {
    if (new_len != m_len) {
        m_data.reset();
        m_len = new_len;
        m_data = m_len > 0 ? std::make_unique<T[]>(m_len) : nullptr;
...
template <class T> void NRVector<T>::assign(size_t new_len, const T &a) {
    if (new_len != m_len) {
        m_data.reset();
        m_len = new_len;
        m_data = m_len > 0 ? std::make_unique<T[]>(m_len) : nullptr;
