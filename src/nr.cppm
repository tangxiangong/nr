...
export struct NRError {
    ...
    std::string file;
    size_t line;
    NRError(const std::string& msg, const std::string& f, size_t l)
...
struct std::formatter<NRError> : std::formatter<std::string> {
    ...
    auto format(const NRError& err, FormatContext& ctx) const {
        return std::format_to(
            ctx.out(),
            "ERROR: {}\n    in file: {} at line: {}", err.message, err.file, err.line);
    }
};
...
template<typename T>
NRVector<T>::NRVector(size_t n) : m_len(n), m_data(n == 0 ? nullptr : std::make_unique<T[]>(n)) {}

template<typename T>
NRVector<T>::NRVector(size_t n, const T& val) : m_len(n), m_data(n == 0 ? nullptr : std::make_unique<T[]>(n)) {
    for (size_t i = 0; i < n; ++i)
        m_data[i] = val;
}

template<typename T>
NRVector<T>::NRVector(size_t n, const T* arr) : m_len(n), m_data(n == 0 ? nullptr : std::make_unique<T[]>(n)) {
    if (arr == nullptr && n > 0)
        throw NRError("Null pointer exception in NRVector constructor", __FILE__, __LINE__);
    for (size_t i = 0; i < n; ++i)
        m_data[i] = arr[i];
}

template<typename T>
NRVector<T>::NRVector(const NRVector& vec) : m_len(vec.m_len), m_data(vec.m_len == 0 ? nullptr : std::make_unique<T[]>(vec.m_len)) {
    for (size_t i = 0; i < m_len; ++i)
        m_data[i] = vec.m_data[i];
}

template<typename T>
NRVector<T>::NRVector(std::initializer_list<T> init) : m_len(init.size()), m_data(init.size() == 0 ? nullptr : std::make_unique<T[]>(init.size())) {
    size_t i = 0;
